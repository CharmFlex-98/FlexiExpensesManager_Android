...
internal interface AccountDao {
    ...
    @Query(
        "SELECT ag.id as account_group_id," +
                ...
                " LEFT JOIN (" +
                " SELECT account_from_id, SUM(t.amount_in_cent * t.rate) as out_amount FROM TransactionEntity t" +
                ...
                " LEFT JOIN (" +
                " SELECT account_to_id, SUM(t.amount_in_cent * t.rate) as in_amount FROM TransactionEntity t" +
                " WHERE ((:startDate IS NULL) OR (date(transaction_date) >= date(:startDate))) AND ((:endDate IS NULL) OR date(transaction_date) <= date(:endDate))" +
